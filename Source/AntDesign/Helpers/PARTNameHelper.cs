namespace AntDesign.Helpers;
public static class PARTNameHelper
{
    //Global
    public static string PART_ContentPresenter => nameof(PART_ContentPresenter);

    //Windows
    public static string PART_TransparencyFallback => nameof(PART_TransparencyFallback);

    //TitleBar
    public static string PART_MouseTracker => nameof(PART_MouseTracker);
    public static string PART_Container => nameof(PART_Container);
    public static string PART_Background => nameof(PART_Background);
    public static string PART_CaptionButtons => nameof(PART_CaptionButtons);

    //CaptionButtons
    public static string PART_FullScreenButton => nameof(PART_FullScreenButton);
    public static string PART_MinimiseButton => nameof(PART_MinimiseButton);
    public static string PART_RestoreButton => nameof(PART_RestoreButton);
    public static string PART_CloseButton => nameof(PART_CloseButton);

    //CheckBox
    public static string PART_Border => nameof(PART_Border);

    //DataValidationErrors
    public static string PART_InlineErrorTemplatePanel => nameof(PART_InlineErrorTemplatePanel);

    //ScrollBar
    public static string PART_LineUpButton => nameof(PART_LineUpButton);
    public static string PART_PageUpButton => nameof(PART_PageUpButton);
    public static string PART_PageDownButton => nameof(PART_PageDownButton);
    public static string PART_LineDownButton => nameof(PART_LineDownButton);

    //ScrollViwer
    public static string PART_HorizontalScrollBar => nameof(PART_HorizontalScrollBar);
    public static string PART_VerticalScrollBar => nameof(PART_VerticalScrollBar);
    public static string PART_ScrollBarsSeparator => nameof(PART_ScrollBarsSeparator);

    //TextBox
    public static string PART_BorderElement => nameof(PART_BorderElement);
    public static string PART_InnerDockPanel => nameof(PART_InnerDockPanel);
    public static string PART_FloatingWatermark => nameof(PART_FloatingWatermark);
    public static string PART_Watermark => nameof(PART_Watermark);
    public static string PART_TextPresenter => nameof(PART_TextPresenter);

    //ButtonSpinner
    public static string PART_SpinnerPanel => nameof(PART_SpinnerPanel);
    public static string PART_IncreaseButton => nameof(PART_IncreaseButton);
    public static string PART_DecreaseButton => nameof(PART_DecreaseButton);

    //AutoCompleteBox
    public static string PART_LayoutRoot => nameof(PART_LayoutRoot);
    public static string PART_TextBox => nameof(PART_TextBox);
    public static string PART_Popup => nameof(PART_Popup);
    public static string PART_SuggestionsContainer => nameof(PART_SuggestionsContainer);
    public static string PART_SelectingItemsControl => nameof(PART_SelectingItemsControl);

    //ItemControl
    public static string PART_ItemsPresenter => nameof(PART_ItemsPresenter);

    //ListBox
    public static string PART_ScrollViewer => nameof(PART_ScrollViewer);

}
